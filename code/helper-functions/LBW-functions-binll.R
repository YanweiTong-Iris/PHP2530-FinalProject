...
binll_per_person = function(df, method='tsb', thresh=.1){
   ...
   n_wks = length(unique(df$combgest))
   wks = unique(df$combgest)
   
   col_idx = grepl(paste(method), colnames(df)); col_idx[which(colnames(df) %in% c('id','combgest'))] = TRUE
   df = df[,col_idx]
   View(df)
   ...
   colnames(logl) = c('method','combgest','logl','merr','fp','fn')
   logl$method = method
   
   for(l in 1:n_wks){
      temp_df = as.data.frame(df %>% filter(combgest == wks[l]))
   ...
   # Add gest age.
   logl$gest_age = NA
   logl$gest_age[1:n_wks] = as.numeric(logl$combgest[1:n_wks])
